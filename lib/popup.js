...
var maxListener = function (evt, restore) {
	var el = this;
	//find .ht-popup-body
	while (el && !el.classList.contains("ht-popup-body")) { el = el.parentNode; }
	if (!el) {
...
var show = function (el, options, cb) {
	...
	if (!elClose) {
		elTool = dom_document_tool.prependHtml(elBody, "<span style='float:right;'></span>");
		elClose = dom_document_tool.prependHtml(elTool,
	...
	if (options.maximizeButton || options.maximized) {
		if (!elMax) {
			elMax = dom_document_tool.prependHtml(elTool,
